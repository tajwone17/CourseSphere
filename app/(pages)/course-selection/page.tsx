...
export default function CourseSelectionPage() {
  ...
  const totalCost = selectedCourses.reduce((sum, course) => sum + course.cost, 0);
  const waiver = 10;
  const costAfterWaiver = totalCost * (1 - waiver / 100);

  return (
    <div className="mx-auto max-w-7xl space-y-8 px-4 py-8">
      <div className="mb-8 text-center" data-aos="fade-down" data-aos-duration="1000">
      ...
      {/* Advisor selection */}
      <div className="rounded-lg border border-gray-800 bg-gray-900 p-6 shadow-xl flex justify-center items-center">
        <div >
        <h2 className="mb-4 text-center text-xl font-semibold text-white">Select Your Advisor</h2>
        <Select
        ...
        {selectedAdvisor === "" && (
          <p className="mt-2 text-sm text-red-400">You must select an advisor before proceeding.</p>
        )}
      </div>
      </div>
          ...
          <table className="min-w-full divide-y divide-gray-800">
            <thead className="bg-gray-800">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase">Instructor</th>
            ...
            <tbody className="divide-y divide-gray-800">
              {selectedCourses.map((course) => (
                <tr key={course.id} className="bg-gray-900">
                  <td className="px-6 py-4 whitespace-nowrap text-white">{course.code}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-white">{course.title}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-white">{course.credit}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-white">{course.instructor}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-white">${course.cost}</td>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <label htmlFor="terms" className="text-gray-400">
                  I confirm that I have reviewed the courses and accept the registration terms
                </label>
              </div>
