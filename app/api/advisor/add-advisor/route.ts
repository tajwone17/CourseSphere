...
export async function POST(request: NextRequest) {
  ...
  const [user]: any = await db.query("SELECT * FROM advisor WHERE email = ?", [
    email,
  ]);
  if (user.length > 0) {
  ...
  const result = await db.query(
    "INSERT INTO advisor (name, email, department_id, phone, password, status) VALUES (?, ?, ?, ?, ?, ?)",
    [name, email, department, phone, password, true],
  );
  if (!result) {
    return new Response("Failed to add ADVISOR", { status: 500 });
  }
  const mailoptions = {
    receiver: email,
  ...
  return new Response("Advisor added successfully", { status: 200 });
}
